...
G_DEFINE_TYPE_WITH_PRIVATE(IndicatorSessionUsersMock,
...
static gboolean my_is_live_session(IndicatorSessionUsers *users G_GNUC_UNUSED) {
...
static void my_activate_user(IndicatorSessionUsers *users, guint uid) {
  g_message("%s %s users %p uid %u FIXME", G_STRLOC, G_STRFUNC, (void *)users,
            uid);
}
...
static GList *my_get_uids(IndicatorSessionUsers *users) {
  ...
  return g_hash_table_get_keys(
      INDICATOR_SESSION_USERS_MOCK(users)->priv->users);
}

static IndicatorSessionUser *my_get_user(IndicatorSessionUsers *self,
                                         guint uid) {
  priv_t *p;
...
indicator_session_users_mock_init(IndicatorSessionUsersMock *self) {
  ...
  g_signal_connect_swapped(
      mock_settings, "changed::is-live-session",
...
void indicator_session_users_mock_add_user(IndicatorSessionUsersMock *self,
  ...
  g_return_if_fail(
      !g_hash_table_contains(self->priv->users, GUINT_TO_POINTER(user->uid)));
