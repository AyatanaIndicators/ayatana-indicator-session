...
/* signals enum */
enum { NAME_LOST, LAST_SIGNAL };

static guint signals[LAST_SIGNAL] = {0};

enum { PROP_0, PROP_MAX_USERS, PROP_LAST };
...
enum { PROFILE_DESKTOP, PROFILE_GREETER, PROFILE_LOCKSCREEN, N_PROFILES };

static const char *const menu_names[N_PROFILES] = {"desktop", "desktop_greeter",
                                                   "desktop_lockscreen"};
...
G_DEFINE_TYPE_WITH_PRIVATE(IndicatorSessionService, indicator_session_service,
                           G_TYPE_OBJECT)
...
static inline void rebuild_header_soon(IndicatorSessionService *self) {
  rebuild_soon(self, SECTION_HEADER);
}
static inline void rebuild_switch_section_soon(IndicatorSessionService *self) {
  rebuild_soon(self, SECTION_SWITCH);
}
static inline void rebuild_logout_section_soon(IndicatorSessionService *self) {
  rebuild_soon(self, SECTION_LOGOUT);
}
static inline void rebuild_session_section_soon(IndicatorSessionService *self) {
...
static inline void rebuild_admin_section_soon(IndicatorSessionService *self) {
...
static gboolean show_user_list(IndicatorSessionService *self) {
  return g_settings_get_boolean(self->priv->indicator_settings,
                                "user-show-menu");
}

static GVariant *action_state_for_header(IndicatorSessionService *self) {
  ...
  show_name =
      g_settings_get_boolean(p->indicator_settings, "show-real-name-on-panel");
  ...
  g_variant_builder_add(
      &b, "{sv}", "tooltip",
      g_variant_new_string(_("Session management, status and user switching")));
...
static void update_header_action(IndicatorSessionService *self) {
  g_simple_action_set_state(self->priv->header_action,
...
static void add_user(IndicatorSessionService *self, guint uid) {
...
static void on_user_added(IndicatorSessionUsers *backend_users G_GNUC_UNUSED,
                          guint uid, gpointer gself) {
  add_user(INDICATOR_SESSION_SERVICE(gself), uid);
}

static void on_user_changed(IndicatorSessionUsers *backend_users G_GNUC_UNUSED,
                            guint uid, gpointer gself) {
  add_user(INDICATOR_SESSION_SERVICE(gself), uid);
}

static void maybe_add_users(IndicatorSessionService *self) {
...
static void user_show_menu_changed(IndicatorSessionService *self) {
  if (show_user_list(self))
...
static void on_user_removed(IndicatorSessionUsers *backend_users G_GNUC_UNUSED,
                            guint uid, gpointer gself) {
...
static const char *get_user_label(const IndicatorSessionUser *user) {
  const char *c;
...
static gboolean
usage_mode_to_action_state(GValue *value, GVariant *variant,
                           __attribute__((unused)) gpointer unused) {
  const gchar *usage_mode = g_variant_get_string(variant, NULL);
  GVariant *ret_var = g_variant_new_boolean(
      g_strcmp0(usage_mode, "Windowed") == 0 ? TRUE : FALSE);
...
action_state_to_usage_mode(const GValue *value,
  ...
  GVariant *ret = g_variant_new_string(
      g_variant_get_boolean(var) == TRUE ? "Windowed" : "Staged");
  return ret;
}
...
static GMenuModel *create_admin_section(IndicatorSessionService *self) {
  GMenu *menu;
  priv_t *p = self->priv;
  menu = g_menu_new();

  gboolean bShowCustom = g_settings_get_boolean(self->priv->indicator_settings,
                                                "show-custom-item");

  if (bShowCustom) {
    gchar *sLabel = g_settings_get_string(self->priv->indicator_settings,
                                          "custom-item-label");
    gchar *sUri = g_settings_get_string(self->priv->indicator_settings,
                                        "custom-item-uri");
  ...
  gboolean bShowDeviceInfo = g_settings_get_boolean(
      self->priv->indicator_settings, "show-device-info");
  ...
  gboolean bShowDesktopHelp = g_settings_get_boolean(
      self->priv->indicator_settings, "show-desktop-help");
  ...
  gboolean bShowDistroHelp = g_settings_get_boolean(
      self->priv->indicator_settings, "show-distro-help");
  ...
  if (p->usage_mode_action && ayatana_common_utils_is_lomiri()) {
    ...
    g_menu_item_set_attribute(menu_item, "x-ayatana-type", "s",
                              "org.ayatana.indicator.switch");
    g_menu_append_item(menu, menu_item);
    g_object_unref(menu_item);
  }

  gboolean bShowBugReport =
      g_settings_get_boolean(self->priv->indicator_settings, "show-bug-report");
...
static GMenuModel *create_settings_section(IndicatorSessionService *self) {
  GMenu *menu;
  priv_t *p = self->priv;
 ...
 */
static GVariant *create_guest_switcher_state(IndicatorSessionService *self) {
  GVariant *val;
  GVariantBuilder b;
 ...
 */
static GVariant *create_user_switcher_state(IndicatorSessionService *self) {
  GVariantBuilder a;
  GVariantBuilder b;
  GVariant *val;
...
static void update_switch_actions(IndicatorSessionService *self) {
...
static gboolean use_ellipsis(IndicatorSessionService *self) {
...
static int compare_users_by_usefulness(gconstpointer ga, gconstpointer gb) {
...
static int compare_users_by_label(gconstpointer ga, gconstpointer gb) {
  int i;
  const IndicatorSessionUser *a = *(const IndicatorSessionUser **)ga;
  const IndicatorSessionUser *b = *(const IndicatorSessionUser **)gb;
...
static GVariant *serialize_icon_file(const gchar *filename) {
...
static void report_unusable_user(IndicatorSessionService *self,
                                 const IndicatorSessionUser *u) {
  const priv_t *const p = self->priv;
  gpointer key;
  ...
  if (!g_hash_table_contains(p->reported_users, key)) {
    ...
    report_recoverable_problem(error_name, (GPid)0, FALSE,
                               (gchar **)additional->pdata);
...
static GMenuModel *create_switch_section(IndicatorSessionService *self,
  ...
  if (indicator_session_users_is_live_session(p->backend_users)) {
    ...
    if (indicator_session_actions_can_switch(p->backend_actions))
      item = g_menu_item_new(
          ellipsis ? _("Switch Account…") : _("Switch Account"), action);
    want_accel = FALSE;
  } else {
  ...
  if (item) {
    if (want_accel) {
      ...
      if (ayatana_common_utils_is_mate()) {
        ...
        gchar **lAccels =
            g_settings_get_strv(p->keybinding_settings, "screensaver");
  ...
  if (indicator_session_guest_is_allowed(p->backend_guest)) {
    ...
    g_menu_item_set_attribute(item, "x-ayatana-type", "s",
                              "org.ayatana.indicator.guest-menu-item");
    g_menu_append_item(menu, item);
  ...
  for (i = 0; i < users->len; ++i) {
    ...
    g_menu_item_set_action_and_target(item, "indicator.switch-to-user", "s",
                                      u->user_name);
    g_menu_item_set_attribute(item, "x-ayatana-type", "s",
                              "org.ayatana.indicator.user-menu-item");

    if ((serialized_icon = serialize_icon_file(u->icon_file))) {
      g_menu_item_set_attribute_value(item, G_MENU_ATTRIBUTE_ICON,
                                      serialized_icon);
...
static GMenuModel *create_logout_section(IndicatorSessionService *self) {
  GMenu *menu;
  const priv_t *const p = self->priv;
  const gboolean ellipsis = use_ellipsis(self);
...
static GMenuModel *create_session_section(IndicatorSessionService *self,
                                          int profile) {
  GMenu *menu;
  const priv_t *const p = self->priv;
  ...
  if (self->priv->bLocal) {
    ...
    if (bSuspendable) {
      gchar *suspend_remote = g_strdup_printf(_("Suspend %s Session"),
...
static void create_menu(IndicatorSessionService *self, int profile) {
  GMenu *menu;
  GMenu *submenu;
  GMenuItem *header;
...
static IndicatorSessionActions *get_backend_actions(gpointer gself) {
  return INDICATOR_SESSION_SERVICE(gself)->priv->backend_actions;
}

static void on_about_activated(GSimpleAction *a G_GNUC_UNUSED,
                               GVariant *param G_GNUC_UNUSED, gpointer gself) {
  indicator_session_actions_about(get_backend_actions(gself));
}

static void on_online_accounts_activated(GSimpleAction *a G_GNUC_UNUSED,
                                         GVariant *param G_GNUC_UNUSED,
                                         gpointer gself) {
  indicator_session_actions_online_accounts(get_backend_actions(gself));
}

static void on_desktop_help_activated(GSimpleAction *a G_GNUC_UNUSED,
                                      GVariant *param G_GNUC_UNUSED,
                                      gpointer gself) {
  indicator_session_actions_desktop_help(get_backend_actions(gself));
}
static void on_distro_help_activated(GSimpleAction *a G_GNUC_UNUSED,
                                     GVariant *param G_GNUC_UNUSED,
                                     gpointer gself) {
  indicator_session_actions_distro_help(get_backend_actions(gself));
}

static void on_bug_activated(GSimpleAction *a G_GNUC_UNUSED,
                             GVariant *param G_GNUC_UNUSED, gpointer gself) {
  indicator_session_actions_bug(get_backend_actions(gself));
}

static void on_settings_activated(GSimpleAction *a G_GNUC_UNUSED,
                                  GVariant *param G_GNUC_UNUSED,
                                  gpointer gself) {
  indicator_session_actions_settings(get_backend_actions(gself));
}

static void on_logout_activated(GSimpleAction *a G_GNUC_UNUSED,
                                GVariant *param G_GNUC_UNUSED, gpointer gself) {
  IndicatorSessionService *self = INDICATOR_SESSION_SERVICE(gself);
...
static void on_suspend_activated(GSimpleAction *a G_GNUC_UNUSED,
                                 GVariant *param G_GNUC_UNUSED,
                                 gpointer gself) {
  indicator_session_actions_suspend(get_backend_actions(gself));
}

static void on_hibernate_activated(GSimpleAction *a G_GNUC_UNUSED,
                                   GVariant *param G_GNUC_UNUSED,
                                   gpointer gself) {
  indicator_session_actions_hibernate(get_backend_actions(gself));
}

static void on_reboot_activated(GSimpleAction *action G_GNUC_UNUSED,
                                GVariant *param G_GNUC_UNUSED, gpointer gself) {
  indicator_session_actions_reboot(get_backend_actions(gself));
}

static void on_power_off_activated(GSimpleAction *a G_GNUC_UNUSED,
                                   GVariant *param G_GNUC_UNUSED,
                                   gpointer gself) {
  indicator_session_actions_power_off(get_backend_actions(gself));
}

static void on_guest_activated(GSimpleAction *a G_GNUC_UNUSED,
                               GVariant *param G_GNUC_UNUSED, gpointer gself) {
  indicator_session_actions_switch_to_guest(get_backend_actions(gself));
}

static void on_screensaver_activated(GSimpleAction *a G_GNUC_UNUSED,
                                     GVariant *param G_GNUC_UNUSED,
                                     gpointer gself) {
  indicator_session_actions_switch_to_screensaver(get_backend_actions(gself));
}

static void on_greeter_activated(GSimpleAction *a G_GNUC_UNUSED,
                                 GVariant *param G_GNUC_UNUSED,
                                 gpointer gself) {
  indicator_session_actions_switch_to_greeter(get_backend_actions(gself));
}

static void on_user_activated(GSimpleAction *a G_GNUC_UNUSED, GVariant *param,
                              gpointer gself) {
...
static void on_custom_activated(GSimpleAction *pAction G_GNUC_UNUSED,
                                GVariant *pParam G_GNUC_UNUSED,
                                gpointer pUserData) {
  IndicatorSessionService *self = INDICATOR_SESSION_SERVICE(pUserData);
  gchar *sUri =
      g_settings_get_string(self->priv->indicator_settings, "custom-item-uri");
  ayatana_common_utils_open_url(sUri);
}

static void on_remote_suspend(GSimpleAction *pAction G_GNUC_UNUSED,
                              GVariant *pParam G_GNUC_UNUSED,
                              gpointer pUserData) {
#ifdef RDA_ENABLED
  rda_session_suspend();
...
static void init_gactions(IndicatorSessionService *self) {
  GVariant *v;
  GSimpleAction *a;
  priv_t *p = self->priv;

  GActionEntry entries[] = {{"about", on_about_activated},
  ...
  g_action_map_add_action_entries(G_ACTION_MAP(p->actions), entries,
  ...
  if (p->usage_mode_settings) {
    a = g_simple_action_new_stateful("usage-mode", NULL,
                                     g_variant_new_boolean(FALSE));
    g_settings_bind_with_mapping(p->usage_mode_settings, "usage-mode", a,
                                 "state", G_SETTINGS_BIND_DEFAULT,
                                 usage_mode_to_action_state,
                                 action_state_to_usage_mode, NULL, NULL);
 ...
 */
static void rebuild_section(GMenu *parent, int pos, GMenuModel *new_section) {
  g_menu_remove(parent, pos);
  g_menu_insert_section(parent, pos, NULL, new_section);
  g_object_unref(new_section);
}

static void rebuild_now(IndicatorSessionService *self, int sections) {
  priv_t *p = self->priv;
  ...
  if (sections & SECTION_SWITCH) {
    rebuild_section(desktop->submenu, 2,
                    create_switch_section(self, PROFILE_DESKTOP));
    rebuild_section(lockscreen->submenu, 0,
                    create_switch_section(self, PROFILE_LOCKSCREEN));
  ...
  if (sections & SECTION_SESSION) {
    rebuild_section(desktop->submenu, 4,
                    create_session_section(self, PROFILE_DESKTOP));
    rebuild_section(greeter->submenu, 0,
                    create_session_section(self, PROFILE_GREETER));
    rebuild_section(lockscreen->submenu, 1,
                    create_session_section(self, PROFILE_LOCKSCREEN));
  }
}

static int rebuild_timeout_func(IndicatorSessionService *self) {
  priv_t *p = self->priv;
...
static void rebuild_soon(IndicatorSessionService *self, int section) {
  ...
  if (p->rebuild_id == 0) {
    ...
    p->rebuild_id = g_timeout_add(REBUILD_INTERVAL_MSEC,
                                  (GSourceFunc)rebuild_timeout_func, self);
  }
}
...
static void on_bus_acquired(GDBusConnection *connection, const gchar *name,
  ...
  /* export the actions */
  if ((id = g_dbus_connection_export_action_group(
           connection, BUS_PATH, G_ACTION_GROUP(p->actions), &err))) {
  ...
  for (i = 0; i < N_PROFILES; ++i) {
    ...
    if ((id = g_dbus_connection_export_menu_model(
             connection, path, G_MENU_MODEL(menu->menu), &err))) {
      menu->export_id = id;
    } else {
...
static void unexport(IndicatorSessionService *self) {
  int i;
  priv_t *p = self->priv;
...
static void on_name_lost(GDBusConnection *connection G_GNUC_UNUSED,
                         const gchar *name, gpointer gself) {
  IndicatorSessionService *self = INDICATOR_SESSION_SERVICE(gself);
...
indicator_session_service_init(IndicatorSessionService *self) {
  ...
  if (ayatana_common_utils_is_mate()) {
    p->keybinding_settings =
        g_settings_new("org.mate.SettingsDaemon.plugins.media-keys");
  } else if (ayatana_common_utils_is_budgie()) {
    p->keybinding_settings =
        g_settings_new("org.gnome.settings-daemon.plugins.media-keys");
    ...
    // Lomiri does not have a keybinding schema. However it has a usage mode
    // schema.
    usage_mode_schema = g_settings_schema_source_lookup(
        g_settings_schema_source_get_default(), usage_mode_schema_name, TRUE);
  ...
  backend_get(p->cancellable, &p->backend_actions, &p->backend_users,
              &p->backend_guest);
  ...
  p->users = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL,
  ...
  g_signal_connect_swapped(gp, "changed::show-device-info",
                           G_CALLBACK(rebuild_admin_section_soon), self);
  g_signal_connect_swapped(gp, "changed::show-desktop-help",
                           G_CALLBACK(rebuild_admin_section_soon), self);
  g_signal_connect_swapped(gp, "changed::show-distro-help",
                           G_CALLBACK(rebuild_admin_section_soon), self);
  g_signal_connect_swapped(gp, "changed::show-bug-report",
                           G_CALLBACK(rebuild_admin_section_soon), self);
  g_signal_connect_swapped(gp, "changed::show-custom-item",
                           G_CALLBACK(rebuild_admin_section_soon), self);
  g_signal_connect_swapped(gp, "changed::custom-item-label",
                           G_CALLBACK(rebuild_admin_section_soon), self);
  g_signal_connect_swapped(gp, "changed::custom-item-uri",
                           G_CALLBACK(rebuild_admin_section_soon), self);
  ...
  self->priv->own_id = g_bus_own_name(
      G_BUS_TYPE_SESSION, BUS_NAME, G_BUS_NAME_OWNER_FLAGS_ALLOW_REPLACEMENT,
...
static void my_get_property(GObject *o, guint property_id, GValue *value,
                            GParamSpec *pspec) {
...
static void my_set_property(GObject *o, guint property_id, const GValue *value,
                            GParamSpec *pspec) {
  IndicatorSessionService *self = INDICATOR_SESSION_SERVICE(o);
...
indicator_session_service_class_init(IndicatorSessionServiceClass *klass) {
  ...
  signals[NAME_LOST] =
      g_signal_new(INDICATOR_SESSION_SERVICE_SIGNAL_NAME_LOST,
  ...
  properties[PROP_MAX_USERS] = g_param_spec_uint(
      "max-users", "Max Users", "Max visible users", 0, INT_MAX, 12,
