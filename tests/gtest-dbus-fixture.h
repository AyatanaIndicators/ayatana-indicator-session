...
private:
  static void on_bus_opened(GObject *o G_GNUC_UNUSED, GAsyncResult *res,
                            gpointer gself) {
  ...
  static void on_bus_closed(GObject *o G_GNUC_UNUSED, GAsyncResult *res,
                            gpointer gself) {
    GTestDBusFixture *self = static_cast<GTestDBusFixture *>(gself);
...
protected:
  ...
  void wait_for_signal(gpointer o, const gchar *signal,
    ...
    const auto handler_id =
        g_signal_connect_swapped(o, signal, G_CALLBACK(g_main_loop_quit), loop);
    const auto timeout_id =
        g_timeout_add_seconds(timeout_seconds, wait_for_signal__timeout, loop);
    g_main_loop_run(loop);
