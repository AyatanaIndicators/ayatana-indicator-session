...
G_DEFINE_TYPE_WITH_PRIVATE(IndicatorSessionUsersDbus,
...
struct UserRecord *user_record_new(AccountsUser *user, gulong signal_id) {
...
static void user_record_free(struct UserRecord *rec) {
...
static AccountsUser *get_user_for_uid(IndicatorSessionUsersDbus *self,
                                      guint uid) {
  struct UserRecord *rec;
...
static gboolean is_tracked_uid(IndicatorSessionUsersDbus *self, guint uid) {
  return get_user_for_uid(self, uid) != NULL;
}

static void emit_user_added(IndicatorSessionUsersDbus *self, guint32 uid) {
  if (is_tracked_uid(self, uid))
    indicator_session_users_added(INDICATOR_SESSION_USERS(self), uid);
}

static void emit_user_changed(IndicatorSessionUsersDbus *self, guint32 uid) {
  if (is_tracked_uid(self, uid))
    indicator_session_users_changed(INDICATOR_SESSION_USERS(self), uid);
}

static void emit_user_removed(IndicatorSessionUsersDbus *self, guint32 uid) {
...
static void set_is_live_session_flag(IndicatorSessionUsersDbus *self,
                                     gboolean b) {
  priv_t *p = self->priv;
  ...
  if (p->is_live != b) {
    ...
    indicator_session_users_notify_is_live_session(
        INDICATOR_SESSION_USERS(self));
  }
}

static void set_active_uid(IndicatorSessionUsersDbus *self, guint uid) {
  priv_t *p = self->priv;
...
static void set_logins(IndicatorSessionUsersDbus *self, GHashTable *logins) {
...
static void on_user_changed(AccountsUser *user, gpointer gself) {
...
static void track_user(IndicatorSessionUsersDbus *self, AccountsUser *user) {
  ...
  g_hash_table_insert(p->uid_to_account, GUINT_TO_POINTER(uid),
...
static void untrack_user(IndicatorSessionUsersDbus *self, const gchar *path) {
  guint uid;
  gpointer key;
  gpointer val;
  GHashTableIter iter;
...
static void on_user_proxy_ready(GObject *o G_GNUC_UNUSED, GAsyncResult *res,
                                gpointer self) {
  GError *err;
  AccountsUser *user;
...
static void create_user_proxy_for_path(IndicatorSessionUsersDbus *self,
  ...
  accounts_user_proxy_new_for_bus(
      ...
      "org.freedesktop.Accounts", path, self->priv->cancellable,
      on_user_proxy_ready, self);
}

/* create proxy objects for everything in Account's user-list */
static void on_user_list_ready(GObject *o, GAsyncResult *res, gpointer gself) {
  GError *err;
  gchar **paths;
...
static void set_account_manager(IndicatorSessionUsersDbus *self, Accounts *a) {
  priv_t *p = self->priv;
  ...
  if (a != NULL) {
    ...
    accounts_call_list_cached_users(a, self->priv->cancellable,
                                    on_user_list_ready, self);

    g_signal_connect_swapped(a, "user-added",
                             G_CALLBACK(create_user_proxy_for_path), self);

    g_signal_connect_swapped(a, "user-deleted", G_CALLBACK(untrack_user), self);
  }
}
...
static void on_login1_manager_session_list_ready(GObject *o, GAsyncResult *res,
                                                 gpointer gself) {
  GVariant *sessions;
  GError *err;
  ...
  sessions = NULL;
  err = NULL;
  login1_manager_call_list_sessions_finish(LOGIN1_MANAGER(o), &sessions, res,
                                           &err);
  ...
  if (err != NULL) {
    ...
    while (g_variant_iter_loop(&iter, "(&su&s&s&o)", &session_id, &uid,
                               &user_name, &seat_id, &path)) {
...
static void update_session_list(IndicatorSessionUsersDbus *self) {
  priv_t *p = self->priv;

  if (p->login1_manager != NULL) {
    login1_manager_call_list_sessions(p->login1_manager, p->cancellable,
                                      on_login1_manager_session_list_ready,
                                      self);
  }
}

static gboolean on_update_session_list_timer(gpointer gself) {
  IndicatorSessionUsersDbus *self = INDICATOR_SESSION_USERS_DBUS(gself);
...
static void update_session_list_twice(IndicatorSessionUsersDbus *self) {
  priv_t *p = self->priv;
  ...
  if (p->update_list_tag == 0)
    p->update_list_tag =
        g_timeout_add_seconds(5, on_update_session_list_timer, self);
}

static void set_login1_manager(IndicatorSessionUsersDbus *self,
                               Login1Manager *login1_manager) {
  priv_t *p = self->priv;
...
static void set_login1_seat(IndicatorSessionUsersDbus *self,
                            Login1Seat *login1_seat) {
  priv_t *p = self->priv;
...
static void set_display_manager_seat(IndicatorSessionUsersDbus *self,
                                     DisplayManagerSeat *dm_seat) {
  priv_t *p = self->priv;
...
static void my_activate_user(IndicatorSessionUsers *users, guint uid) {
...
static gboolean my_is_live_session(IndicatorSessionUsers *users) {
...
static GList *my_get_uids(IndicatorSessionUsers *users) {
  IndicatorSessionUsersDbus *self = INDICATOR_SESSION_USERS_DBUS(users);
  return g_hash_table_get_keys(self->priv->uid_to_account);
}

/* build a new struct populated with info on the specified user */
static IndicatorSessionUser *my_get_user(IndicatorSessionUsers *users,
                                         guint uid) {
  IndicatorSessionUsersDbus *self = INDICATOR_SESSION_USERS_DBUS(users);
  priv_t *p = self->priv;
...
static void indicator_session_users_dbus_init(IndicatorSessionUsersDbus *self) {
  priv_t *p;
  ...
  p->uid_to_account = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL,
...
void indicator_session_users_dbus_set_proxies(IndicatorSessionUsersDbus *self,
                                              Login1Manager *login1_manager,
                                              Login1Seat *login1_seat,
                                              DisplayManagerSeat *dm_seat,
                                              Accounts *accounts) {
