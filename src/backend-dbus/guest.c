...
G_DEFINE_TYPE_WITH_PRIVATE(IndicatorSessionGuestDbus,
...
static void set_guest_is_allowed_flag(IndicatorSessionGuestDbus *self,
                                      gboolean b) {
  priv_t *p = self->priv;
...
static void set_guest_is_logged_in_flag(IndicatorSessionGuestDbus *self,
                                        gboolean b) {
  priv_t *p = self->priv;
...
static void set_guest_is_active_flag(IndicatorSessionGuestDbus *self,
                                     gboolean b) {
  priv_t *p = self->priv;
...
static void on_login1_manager_session_list_ready(GObject *o, GAsyncResult *res,
  ...
  if (err != NULL) {
    ...
    while (g_variant_iter_loop(&iter, "(&su&s&so)", &session_id, &uid,
                               &user_name, &seat_id, NULL)) {
      ...
      is_guest = g_str_has_prefix(user_name, "guest-") && (uid < 1000);
...
static void update_session_list(IndicatorSessionGuestDbus *self) {
  priv_t *p = self->priv;
...
static void set_login1_manager(IndicatorSessionGuestDbus *self,
                               Login1Manager *login1_manager) {
  priv_t *p = self->priv;
...
static void set_login1_seat(IndicatorSessionGuestDbus *self,
                            Login1Seat *login1_seat) {
  priv_t *p = self->priv;
...
static void on_switch_to_guest_done(GObject *o, GAsyncResult *res,
                                    gpointer unused G_GNUC_UNUSED) {
  GError *err;

  err = NULL;
  display_manager_seat_call_switch_to_guest_finish(DISPLAY_MANAGER_SEAT(o), res,
                                                   &err);
  if (err != NULL) {
    if (!g_error_matches(err, G_IO_ERROR, G_IO_ERROR_CANCELLED))
...
static void my_switch_to_guest(IndicatorSessionGuest *guest) {
  ...
  display_manager_seat_call_switch_to_guest(self->priv->dm_seat, "",
                                            ...
                                            on_switch_to_guest_done, self);
}

static void on_notify_has_guest_account(DisplayManagerSeat *dm_seat,
...
static void set_display_manager_seat(IndicatorSessionGuestDbus *self,
                                     DisplayManagerSeat *dm_seat) {
  priv_t *p = self->priv;
...
static void indicator_session_guest_dbus_init(IndicatorSessionGuestDbus *self) {
  priv_t *p;
