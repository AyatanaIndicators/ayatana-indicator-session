...
static void on_proxy_ready_impl(struct session_proxy_data *data,
                                gsize member_offset, GError *err,
                                gpointer proxy) {
  if (err != NULL) {
  ...
  if (!--data->pending) {
    data->callback(data->login1_manager, data->login1_seat, data->dm_seat,
                   data->account_manager, data->cancellable, data->user_data);
...
static void on_display_manager_seat_proxy_ready(GObject *o G_GNUC_UNUSED,
                                                GAsyncResult *res,
                                                gpointer gdata) {
...
static void on_login1_seat_ready(GObject *o G_GNUC_UNUSED, GAsyncResult *res,
                                 gpointer gdata) {
...
static void on_login1_manager_ready(GObject *o G_GNUC_UNUSED, GAsyncResult *res,
                                    gpointer gdata) {
...
static void on_accounts_proxy_ready(GObject *o G_GNUC_UNUSED, GAsyncResult *res,
                                    gpointer gdata) {
...
void indicator_session_util_get_session_proxies(
  ...
  login1_manager_proxy_new_for_bus(
      ...
      "org.freedesktop.login1", "/org/freedesktop/login1", data->cancellable,
  ...
  if ((str = g_getenv("XDG_SEAT"))) {
    ...
    login1_seat_proxy_new_for_bus(
        G_BUS_TYPE_SYSTEM, G_DBUS_PROXY_FLAGS_GET_INVALIDATED_PROPERTIES,
        "org.freedesktop.login1", path, data->cancellable, on_login1_seat_ready,
        data);
    g_free(path);
  }
  ...
  accounts_proxy_new_for_bus(
      ...
      data->cancellable, on_accounts_proxy_ready, data);
  ...
  if ((str = g_getenv("XDG_SEAT_PATH"))) {
    ...
    display_manager_seat_proxy_new_for_bus(
        G_BUS_TYPE_SYSTEM, G_DBUS_PROXY_FLAGS_GET_INVALIDATED_PROPERTIES,
        "org.freedesktop.DisplayManager", str, data->cancellable,
