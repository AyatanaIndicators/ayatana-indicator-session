...
G_DEFINE_TYPE(IndicatorSessionGuest, indicator_session_guest, G_TYPE_OBJECT)

enum { PROP_0, PROP_ALLOWED, PROP_LOGGED_IN, PROP_ACTIVE, PROP_LAST };
...
static void my_dispose(GObject *object) {
...
indicator_session_guest_class_init(IndicatorSessionGuestClass *klass) {
  ...
  properties[PROP_ALLOWED] = g_param_spec_boolean(
      INDICATOR_SESSION_GUEST_PROPERTY_ALLOWED, "Is Allowed",
      "Whether or not a Guest user is allowed", FALSE, flags);

  properties[PROP_LOGGED_IN] = g_param_spec_boolean(
      INDICATOR_SESSION_GUEST_PROPERTY_LOGGED_IN, "Is Logged In",
      "Whether or not the Guest account is logged in", FALSE, flags);

  properties[PROP_ACTIVE] = g_param_spec_boolean(
      INDICATOR_SESSION_GUEST_PROPERTY_ACTIVE, "Is Active",
      "If the Guest account has the current session", FALSE, flags);
...
****
***/
static void notify_func(IndicatorSessionGuest *self, int prop) {
