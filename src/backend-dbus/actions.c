...
G_DEFINE_TYPE_WITH_PRIVATE(IndicatorSessionActionsDbus,
...
static void log_and_clear_error(GError **err, const char *loc,
                                const char *func) {
  if (*err) {
...
static prompt_status_t get_prompt_status(IndicatorSessionActionsDbus *self) {
  prompt_status_t prompt = PROMPT_NONE;
  const priv_t *p = self->priv;

  if (!g_settings_get_boolean(p->indicator_settings,
                              "suppress-logout-restart-shutdown")) {
    /* can we use the MATE prompt? */
    if ((prompt == PROMPT_NONE) &&
        ayatana_common_utils_have_mate_program("mate-session-save"))
      prompt = PROMPT_WITH_MATE;

    // can we use the BUDGIE (currently GNOME) prompt?
    if ((prompt == PROMPT_NONE) &&
        ayatana_common_utils_have_budgie_program("gnome-session-quit")) {
      prompt = PROMPT_WITH_BUDGIE;
    }

    /* can we use the XFCE prompt? */
    if ((prompt == PROMPT_NONE) &&
        ayatana_common_utils_have_xfce_program("xfce4-session-logout"))
...
static void on_seat_notify_multi_session(IndicatorSessionActionsDbus *self) {
  priv_t *p = self->priv;
  gboolean b;
  ...
  if (p->seat_allows_activation != b) {
    ...
    indicator_session_actions_notify_can_switch(
        INDICATOR_SESSION_ACTIONS(self));
  }
}

static void set_login1_seat(IndicatorSessionActionsDbus *self,
                            Login1Seat *seat) {
  priv_t *p = self->priv;
...
static void set_dm_seat(IndicatorSessionActionsDbus *self,
...
static void on_screensaver_proxy_ready(GObject *o G_GNUC_UNUSED,
                                       GAsyncResult *res, gpointer gself) {
  GError *err;
  GnomeScreenSaver *ss;
...
static void on_lomiri_proxy_ready(GObject *o G_GNUC_UNUSED, GAsyncResult *res,
                                  gpointer gself) {
  GError *err;
...
static void on_can_suspend_ready(GObject *o, GAsyncResult *res,
                                 gpointer gself) {
  char *str;
  GError *err;
...
static void on_can_hibernate_ready(GObject *o, GAsyncResult *res,
                                   gpointer gself) {
  gchar *str;
  GError *err;
...
static void set_login1_manager(IndicatorSessionActionsDbus *self,
                               Login1Manager *login1_manager) {
  priv_t *p = self->priv;
  ...
  if (login1_manager != NULL) {
    ...
    login1_manager_call_can_suspend(p->login1_manager,
                                    ...
                                    on_can_suspend_ready, self);

    login1_manager_call_can_hibernate(p->login1_manager,
                                      p->login1_manager_cancellable,
                                      on_can_hibernate_ready, self);
  }
}

static void on_session_manager_proxy_ready(GObject *o G_GNUC_UNUSED,
                                           GAsyncResult *res, gpointer gself) {
  GError *err;
...
static void on_webcredentials_proxy_ready(GObject *o G_GNUC_UNUSED,
                                          GAsyncResult *res, gpointer gself) {
  GError *err;
  Webcredentials *webcredentials;
  ...
  if (err == NULL) {
    INDICATOR_SESSION_ACTIONS_DBUS(gself)->priv->webcredentials =
        webcredentials;

    g_signal_connect_swapped(
        webcredentials, "notify::error-status",
        G_CALLBACK(indicator_session_actions_notify_has_online_account_error),
        gself);
...
static void on_end_session_dialog_proxy_ready(GObject *o G_GNUC_UNUSED,
                                              GAsyncResult *res,
                                              gpointer gself) {
  GError *err;
  EndSessionDialog *end_session_dialog;
  ...
  if (err == NULL) {
    INDICATOR_SESSION_ACTIONS_DBUS(gself)->priv->end_session_dialog =
        end_session_dialog;

    g_signal_connect_swapped(
        end_session_dialog, "notify::g-name-owner",
        G_CALLBACK(indicator_session_actions_notify_can_switch), gself);

    indicator_session_actions_notify_can_prompt(
        INDICATOR_SESSION_ACTIONS(gself));
    indicator_session_actions_notify_can_reboot(
        INDICATOR_SESSION_ACTIONS(gself));
  }
...
static gboolean my_can_reboot(IndicatorSessionActions *actions) {
...
static gboolean my_can_switch(IndicatorSessionActions *self) {
  const priv_t *p = INDICATOR_SESSION_ACTIONS_DBUS(self)->priv;

  return p->seat_allows_activation &&
         !g_settings_get_boolean(p->lockdown_settings,
                                 "disable-user-switching");
}

static gboolean my_can_suspend(IndicatorSessionActions *self) {
  const priv_t *p = INDICATOR_SESSION_ACTIONS_DBUS(self)->priv;

  return p && p->can_suspend;
}

static gboolean my_can_hibernate(IndicatorSessionActions *self) {
  const priv_t *p = INDICATOR_SESSION_ACTIONS_DBUS(self)->priv;

  return p && p->can_hibernate;
}

static gboolean my_can_prompt(IndicatorSessionActions *self) {
  return get_prompt_status(INDICATOR_SESSION_ACTIONS_DBUS(self)) != PROMPT_NONE;
}

static gboolean my_has_online_account_error(IndicatorSessionActions *self) {
  const priv_t *p = INDICATOR_SESSION_ACTIONS_DBUS(self)->priv;

  return p && (p->webcredentials) &&
         (webcredentials_get_error_status(p->webcredentials));
}

static void my_suspend(IndicatorSessionActions *self) {
  priv_t *p = INDICATOR_SESSION_ACTIONS_DBUS(self)->priv;

  g_return_if_fail(p->login1_manager != NULL);

  login1_manager_call_suspend(p->login1_manager, FALSE,
                              p->login1_manager_cancellable, NULL, NULL);
}

static void my_hibernate(IndicatorSessionActions *self) {
  priv_t *p = INDICATOR_SESSION_ACTIONS_DBUS(self)->priv;

  g_return_if_fail(p->login1_manager != NULL);

  login1_manager_call_hibernate(p->login1_manager, FALSE,
                                p->login1_manager_cancellable, NULL, NULL);
}
...
static gboolean is_owned_proxy(gpointer proxy) {
...
static void on_gnome_logout_response(GObject *o, GAsyncResult *res,
...
logout_now_gnome_session_manager(IndicatorSessionActionsDbus *self) {
  ...
  if (is_owned_proxy(p->session_manager)) {
    ...
    gnome_session_manager_call_logout(p->session_manager, 1, /* don't prompt */
                                      p->cancellable, on_gnome_logout_response,
                                      self);
    logout_called = TRUE;
  }
...
static void on_lomiri_logout_response(GObject *o, GAsyncResult *res,
                                      gpointer gself) {
  GError *error;

  error = NULL;
  lomiri_shell_session_call_request_logout_finish(LOMIRI_SHELL_SESSION(o), res,
                                                  &error);
...
static gboolean logout_now_desktop(IndicatorSessionActionsDbus *self) {
  priv_t *p = self->priv;
  ...
  if (is_owned_proxy(p->lomiri_session)) {
    ...
    lomiri_shell_session_call_request_logout(p->lomiri_session, p->cancellable,
                                             on_lomiri_logout_response, self);
  }

  return called;
}

static void logout_now(IndicatorSessionActionsDbus *self) {
  if (!logout_now_desktop(self) && !logout_now_gnome_session_manager(self)) {
    g_critical("%s can't logout: no Unity nor GNOME/MATE session proxy",
               G_STRFUNC);
  }
}

static void on_reboot_response(GObject *o, GAsyncResult *res,
                               gpointer unused G_GNUC_UNUSED) {
  GError *err = NULL;
...
static void reboot_now(IndicatorSessionActionsDbus *self) {
  priv_t *p = self->priv;

  g_return_if_fail(p->login1_manager != NULL);

  login1_manager_call_reboot(p->login1_manager, FALSE,
                             p->login1_manager_cancellable, on_reboot_response,
                             NULL);
}

static void power_off_now(IndicatorSessionActionsDbus *self) {
  priv_t *p = self->priv;

  g_return_if_fail(p->login1_manager != NULL);

  login1_manager_call_power_off(p->login1_manager, FALSE,
                                p->login1_manager_cancellable, NULL, NULL);
}

static void stop_listening_to_dialog(IndicatorSessionActionsDbus *self) {
  g_signal_handlers_disconnect_by_data(self->priv->end_session_dialog, self);
}
static void on_end_session_dialog_canceled(IndicatorSessionActionsDbus *self) {
  stop_listening_to_dialog(self);
}
static void on_end_session_dialog_closed(IndicatorSessionActionsDbus *self) {
  stop_listening_to_dialog(self);
}

static void on_open_end_session_dialog_ready(GObject *o, GAsyncResult *res,
...
static void show_lomiri_end_session_dialog(IndicatorSessionActionsDbus *self,
                                           int type) {
  priv_t *p = INDICATOR_SESSION_ACTIONS_DBUS(self)->priv;
  ...
  g_signal_connect_swapped(o, "confirmed-shutdown", G_CALLBACK(power_off_now),
                           self);
  g_signal_connect_swapped(o, "canceled",
                           G_CALLBACK(on_end_session_dialog_canceled), self);
  g_signal_connect_swapped(o, "closed",
                           G_CALLBACK(on_end_session_dialog_closed), self);

  end_session_dialog_call_open(p->end_session_dialog, type, 0, 0,
                               inhibitor_paths, p->cancellable,
                               on_open_end_session_dialog_ready, self);
}

static void my_bug(IndicatorSessionActions *self G_GNUC_UNUSED) {
  if (!ayatana_common_utils_open_url(get_distro_bts_url()))
    ayatana_common_utils_zenity_warning(
        "dialog-warning", _("Warning"),
        _("The operating system's bug tracker needs to be accessed with\na web "
          "browser.\n\nThe Ayatana Session Indicator could not find any "
          "web\nbrowser on your computer."));
}

static void my_logout(IndicatorSessionActions *actions) {
  IndicatorSessionActionsDbus *self = INDICATOR_SESSION_ACTIONS_DBUS(actions);
  ...
  case PROMPT_WITH_UTILS: {
    const char *primary =
        _("Are you sure you want to close all programs and log out?");
    const char *secondary = _("Some software updates won't be applied until "
                              "the computer next restarts.");
    char *text =
        g_strdup_printf("<big><b>%s</b></big>\n \n%s", primary, secondary);

    gboolean confirmed = ayatana_common_utils_zenity_question(
        "system-log-out", _("Log Out"), text, _("Log Out"), _("Cancel"));
    g_free(text);
...
static void my_reboot(IndicatorSessionActions *actions) {
  IndicatorSessionActionsDbus *self = INDICATOR_SESSION_ACTIONS_DBUS(actions);
  ...
  case PROMPT_WITH_MATE:
    /* --shutdown-dialog presents Restart, Shutdown and (if available) Suspend
     * options */
  ...
  case PROMPT_WITH_UTILS:
    if (ayatana_common_utils_zenity_question(
            "system-restart", _("Restart"),
            _("Are you sure you want to close all programs and restart the "
              "computer?"),
            _("Restart"), _("Cancel"))) {
      reboot_now(self);
    }
...
static void my_power_off(IndicatorSessionActions *actions) {
  IndicatorSessionActionsDbus *self = INDICATOR_SESSION_ACTIONS_DBUS(actions);
  ...
  case PROMPT_WITH_UTILS:
    if (ayatana_common_utils_zenity_question(
            "system-shutdown", _("Shut Down"),
            _("Are you sure you want to close all programs and shut down the "
              "computer?"),
            _("Shut Down"), _("Cancel"))) {
      power_off_now(self);
    }
  ...
  if (g_strcmp0(get_desktop_session(), "ubuntu-touch") == 0)
    ...
    ayatana_common_utils_zenity_warning(
        ...
        _("The Ayatana Session Indicator does not know yet, how to show\nthe "
          "currently running desktop's user guide or help center.\n\nPlease "
          "report this to the developers "
          "at:\nhttps://github.com/ArcticaProject/ayatana-indicator-session/"
          "issues"));
}
  ...
  if (!ayatana_common_utils_open_url(get_distro_url()))
    ayatana_common_utils_zenity_warning(
        ...
        g_strdup_printf(_("Displaying information on %s  requires\na web "
                          "browser.\n\nThe Ayatana Session Indicator could not "
                          "find any web\nbrowser on your computer."),
                        get_distro_name()));
}

static void my_settings(IndicatorSessionActions *self G_GNUC_UNUSED) {
  if (ayatana_common_utils_is_lomiri())
    ...
    ayatana_common_utils_zenity_warning(
        "dialog-warning", _("Warning"),
        _("The Ayatana Session Indicator does not support evoking the "
...
static void my_online_accounts(IndicatorSessionActions *self G_GNUC_UNUSED) {
  if (ayatana_common_utils_is_lomiri())
    ...
    ayatana_common_utils_zenity_warning(
        "dialog-warning", _("Warning"),
        _("The Ayatana Session Indicator does not support password "
...
static void my_about(IndicatorSessionActions *self G_GNUC_UNUSED) {
  if (ayatana_common_utils_is_lomiri())
    ...
    ayatana_common_utils_execute_command(
        "mate-system-monitor --show-system-tab");
  else if (ayatana_common_utils_have_xfce_program("xfce4-about"))
    ayatana_common_utils_execute_command("xfce4-about");
  else
    ayatana_common_utils_zenity_warning(
        "dialog-warning", _("Warning"),
        _("The Ayatana Session Indicator does not know yet, how to "
          "show\ninformation of the currently running desktop "
          "environment.\n\nPlease report this to the developers "
          "at:\nhttps://github.com/ArcticaProject/ayatana-indicator-session/"
          "issues"));
}
...
static void lock_current_session(IndicatorSessionActions *self,
                                 gboolean immediate) {
  priv_t *p = INDICATOR_SESSION_ACTIONS_DBUS(self)->priv;

  if (is_owned_proxy(p->lomiri_session)) {
    if (immediate) {
      lomiri_shell_session_call_prompt_lock(p->lomiri_session, p->cancellable,
                                            NULL, NULL);
    } else {
      lomiri_shell_session_call_lock(p->lomiri_session, p->cancellable, NULL,
                                     NULL);
    }
  } else if (ayatana_common_utils_have_mate_program(
                 "mate-screensaver-command")) {
    ayatana_common_utils_execute_command("mate-screensaver-command --lock");
  } else if (ayatana_common_utils_have_budgie_program(
...
static void my_switch_to_screensaver(IndicatorSessionActions *self) {
  lock_current_session(self, TRUE);
}

static void my_switch_to_greeter(IndicatorSessionActions *self) {
  priv_t *p = INDICATOR_SESSION_ACTIONS_DBUS(self)->priv;
...
static void my_switch_to_guest(IndicatorSessionActions *self) {
  priv_t *p = INDICATOR_SESSION_ACTIONS_DBUS(self)->priv;
  ...
  display_manager_seat_call_switch_to_guest(p->dm_seat, "",
                                            p->dm_seat_cancellable, NULL, NULL);
}

static void my_switch_to_username(IndicatorSessionActions *self,
                                  const char *username) {
  priv_t *p = INDICATOR_SESSION_ACTIONS_DBUS(self)->priv;
...
indicator_session_actions_dbus_class_init(
    IndicatorSessionActionsDbusClass *klass) {
...
indicator_session_actions_dbus_init(IndicatorSessionActionsDbus *self) {
  ...
  g_signal_connect_swapped(
      s, "changed::suppress-restart-menuitem",
      G_CALLBACK(indicator_session_actions_notify_can_reboot), self);
  g_signal_connect_swapped(
      s, "changed::suppress-shutdown-menuitem",
      G_CALLBACK(indicator_session_actions_notify_can_reboot), self);
  g_signal_connect_swapped(
      s, "changed::force-restart-menuitem",
      G_CALLBACK(indicator_session_actions_notify_can_reboot), self);
  p->indicator_settings = s;

  gnome_screen_saver_proxy_new_for_bus(
      G_BUS_TYPE_SESSION, G_DBUS_PROXY_FLAGS_NONE, "org.gnome.ScreenSaver",
      "/org/gnome/ScreenSaver", p->cancellable, on_screensaver_proxy_ready,
      self);

  lomiri_shell_session_proxy_new_for_bus(
      G_BUS_TYPE_SESSION, G_DBUS_PROXY_FLAGS_NONE, "com.lomiri.Shell",
      "/com/lomiri/Shell/Session", p->cancellable, on_lomiri_proxy_ready, self);

  gnome_session_manager_proxy_new_for_bus(
      G_BUS_TYPE_SESSION, G_DBUS_PROXY_FLAGS_NONE, "org.gnome.SessionManager",
      "/org/gnome/SessionManager", p->cancellable,
      on_session_manager_proxy_ready, self);
  ...
  webcredentials_proxy_new_for_bus(
      ...
      "/com/lomiri/indicators/webcredentials", p->cancellable,
      on_webcredentials_proxy_ready, self);

  end_session_dialog_proxy_new_for_bus(
      G_BUS_TYPE_SESSION, G_DBUS_PROXY_FLAGS_DO_NOT_AUTO_START,
      "com.lomiri.Shell", "/org/gnome/SessionManager/EndSessionDialog",
      p->cancellable, on_end_session_dialog_proxy_ready, self);
}
...
void indicator_session_actions_dbus_set_proxies(
    IndicatorSessionActionsDbus *self, Login1Manager *login1_manager,
    Login1Seat *login1_seat, DisplayManagerSeat *dm_seat) {
